...
// Tunable display mapping (the original fixed-point scale is not reproduced;
// these are chosen so the object matches the source's on-screen size — see
// README "approximations"). FY keeps the 213/256 VGA pixel-aspect.
const FX = 430; // horizontal focal length (px · unit / view-unit)
const FY = (FX * 213) / 256;
const Z_MIN = 0.25; // near clamp in view units
...
export class GlenzRenderer {
  ...
  drawObject(s: ObjectState): void {
    ...
    for (const t of s.tris) {
      this.drawTri(proj[t.a]!, proj[t.b]!, proj[t.c]!, t.group, s.brightness);
    }
  }

  private drawTri(p0: PV, p1: PV, p2: PV, group: number, fade: number): void {
    ...
    const shade = (0.32 + 0.68 * facing) * fade;
  ...
  private fillTriangle(
    ...
    for (let y = yStart; y <= yEnd; y++) {
      ...
      for (let x = xl; x <= xr; x++) this.vga.addPixel(x, y, r, g, b);
    }
  }
}
