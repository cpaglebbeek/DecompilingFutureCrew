...
// backpal[] in the original is the first 16 palette entries of FC.UH
// (fc[0x10 + a*3], 6-bit) — a PURPLE/VIOLET gradient, *not* an RGB rainbow.
// demo_glz shades each face by face-on-ness (0..63) and `test bp,2` chooses
// between a blue-violet "glass" tint and a near-white/grey tint. We model that
// with two 8-bit base tints, modulated by per-triangle facing in the renderer.
export const GLENZ_TINT_BLUE: [number, number, number] = [70, 90, 255]; // violet glass
export const GLENZ_TINT_WHITE: [number, number, number] = [225, 225, 245]; // pale highlight

// Pick the base tint for a triangle from its original colour-code bit1.
export function tintForCode(code: number): [number, number, number] {
  return code & 2 ? GLENZ_TINT_BLUE : GLENZ_TINT_WHITE;
}
